...
beforeEach(() => {
  ...
  // No prior log for this comment, so the worker takes the create path; and no
  // earlier opening DM, so {username} personalization falls back to null.
...
describe("DM Worker — Full Pipeline", () => {
  ...
  it("should requeue and release monthly usage when rate limited", async () => {
    ...
    expect(mockQueueAdd).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        delay: 1800000,
        // Underscore-separated, not colon-separated: BullMQ namespaces its own
        // Redis keys with colons, so keeping them out of the job id avoids
        // ambiguity when reading keys directly.
